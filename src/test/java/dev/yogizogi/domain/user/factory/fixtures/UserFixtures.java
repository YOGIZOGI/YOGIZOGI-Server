package dev.yogizogi.domain.user.factory.fixtures;

import dev.yogizogi.domain.user.model.entity.User;
import dev.yogizogi.global.util.UuidUtils;
import java.util.UUID;

public class UserFixtures {

    public static UUID 식별자 = UUID.fromString("11ee5886-0ec1-126a-83a9-83536d67b154");
    public static String 계정 = "test1234";
    public static String 닉네임 = "test";
    public static String 비밀번호 = "test1234!";
    public static String 암호화_비밀번호 = "{bcrypt}$2a$10$Fm2mY0xaZRIh97vS8oP5WOFaWgdZpUQkBrTY1OK9NJuDGYN/m.uWq";
    public static String 핸드폰번호 = "01012345678";
    public static String 역할 = "ROLE_USER";

}
